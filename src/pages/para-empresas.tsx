...
export default function ForCompany() {
  ...
  useEffect(()=>{
    if(course){
      if(COMPANY_COURSES.some(comp=> comp.title.toLowerCase().includes(String(course).toLowerCase()))){
        handleOpenModal(COMPANY_COURSES.find(comp=> comp.title.toLowerCase().includes(String(course).toLowerCase())))
      }
    }
  },[course])
