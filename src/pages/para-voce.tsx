...
import { FiBookOpen, FiFile, FiFileText, FiPocket, FiVideo } from "react-icons/fi";
...
export default function ForCompany() {
  ...
  const renderItem = useCallback((item: ListPeopleCourseItem)=> <PeopleCourseItem onOpen={handleOpenModal} item={item}/>,[])
  return (
    <>
      <Head>
      ...
      <Box width='100%' >
        <Stack spacing={4} paddingTop={36} paddingBottom={24} bgGradient={`linear-gradient(60deg,  ${colors[Math.floor(Math.random()*colors.length)]}, ${colors[Math.floor(Math.random()*colors.length)]}, ${colors[Math.floor(Math.random()*colors.length)]})`} textAlign='center' alignItems='center' paddingX={[4, 12, 56]} justifyContent='center' width='100%' >
          ...
          Estude a hora que quiser e de onde quiser, tenha seu certificado agora mesmo e esteja preparado para o mercado de trabalho. 
          </Heading>
        </Stack>
        <Box padding={12}  width={'100%'}>
          ...
          Nossos treinamentos online contam com os melhores profissionais do mercado. Não perca mais tempo e tenha seu certificado agora mesmo. 
          </Text>
